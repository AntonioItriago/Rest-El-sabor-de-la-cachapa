            ...
            <main className="max-w-7xl mx-auto p-4 w-full">
                ...
                <section className="mb-8">
                    ...
                    {Object.keys(billRequestTables).length === 0 ? (
                        <div className="text-center py-8 bg-gray-50 rounded-lg"><p className="text-gray-500">No hay mesas solicitando la cuenta.</p></div>
                    ) : (
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <section className="mb-8">
                    <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">Pedidos Pendientes de Aprobación ({pendingOrders.length})</h2>
                    {pendingOrders.length === 0 ? (
                        <div className="text-center py-8 bg-gray-50 rounded-lg"><p className="text-gray-500">No hay pedidos pendientes.</p></div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <section>
                    <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">Pedidos Activos e Historial ({activeOrders.length})</h2>
                    {activeOrders.length === 0 ? (
                         <div className="text-center py-8 bg-gray-50 rounded-lg"><p className="text-gray-500">No hay pedidos activos.</p></div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {activeOrders.map(order => (
                                <div key={order.id} className="bg-white rounded-lg shadow p-4 border flex flex-col justify-between transition-shadow hover:shadow-md">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600">Mesonero: <strong>{order.waiterId}</strong></p>
                                        
                                        <ul className="text-sm text-gray-600 list-disc list-inside my-2 pl-2">
                                        ...
                                        <p className="text-xs text-gray-400 mt-2">{new Date(order.timestamp).toLocaleString()}</p>
                                    </div>
                                    {order.status === OrderStatus.APPROVED && (
                                        <button 
                                            onClick={() => handleDeliver(order.id)}
                                            disabled={order.waiterId !== currentWaiterId}
                                            className="mt-4 w-full py-2 bg-blue-500 text-white font-bold rounded-lg transition shadow-md active:scale-95 disabled:bg-gray-400 disabled:cursor-not-allowed"
