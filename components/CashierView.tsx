...
const CashierView: React.FC<CashierViewProps> = ({ sessionInfo, onExit, orders, onClearTable, onReassignTable, waiters, setWaiters }) => {
    ...
    const getStatusChipColor = (status: OrderStatus) => {
        const colors: Record<OrderStatus, string> = {
            ...
            [OrderStatus.APPROVED]: 'bg-green-200 text-green-800',
            [OrderStatus.DELIVERED]: 'bg-blue-200 text-blue-800',
            [OrderStatus.BILL_REQUESTED]: 'bg-red-200 text-red-800',
            [OrderStatus.PAID]: 'bg-purple-200 text-purple-800',
        };
                ...
                {!selectedTable ? (
                     <section>
                        <div className="flex justify-between items-center mb-4 border-b pb-2">
                                <h2 className="text-2xl font-bold text-gray-800">Gestión de Mesas Activas</h2>
                        ...
                        {Object.keys(activeTables).length === 0 ? (
                            <div className="text-center py-8 bg-gray-50 rounded-lg"><p className="text-gray-500">No hay mesas con consumo activo.</p></div>
                        ) : (
                    ...
                    selectedTableData && (
                        <section className="bg-white rounded-lg shadow p-6 border animate-fade-in">
                           <div className="flex justify-between items-center border-b pb-4 mb-4">
                                <h2 className="font-bold text-2xl">Mesa {selectedTable}</h2>
                                <button onClick={() => setSelectedTable(null)} className="py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition flex items-center text-sm">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                                <div>
                                    <p className="text-sm text-gray-600">Mesero Actual: <span className="font-semibold">{selectedTableData.waiterId}</span></p>
                                    {selectedTableData.clientName && <p className="text-sm text-gray-600">Cliente: <span className="font-semibold">{selectedTableData.clientName}</span></p>}
                                    <div className="text-sm text-gray-600 mt-2">
                                        <span className="text-lg">Total a Pagar:</span>
                                        <p className="font-extrabold text-3xl text-gray-900">{formatPrice(selectedTableData.total, 'USD')}</p>
                                ...
                                <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
                                    <button onClick={() => handleOpenReassignModal(selectedTable, selectedTableData.waiterId)} className="py-2 px-4 text-sm bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition w-full md:w-auto">Reasignar</button>
                                    <button 
                                        onClick={() => setCloseTableModal({open: true, tableNumber: selectedTable})} 
                                        className="py-2 px-6 bg-purple-500 text-white font-bold rounded-lg transition shadow-md active:scale-95 w-full md:w-auto disabled:bg-gray-400 disabled:cursor-not-allowed"
                            ...
                            <h3 className="font-bold text-lg mb-2 border-t pt-4">Detalle de Pedidos</h3>
                ...
                <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full animate-fade-in">
                        <div className="p-4 border-b">
                            <h3 className="text-lg font-bold text-gray-800">Confirmar Cierre de Mesa</h3>
                        </div>
                        <div className="p-6">
                            <p className="text-gray-700" dangerouslySetInnerHTML={{ __html: `¿Está seguro de que desea cobrar y cerrar la mesa <strong>#${closeTableModal.tableNumber}</strong>? Esta acción es irreversible y eliminará todos sus pedidos.` }}></p>
                        </div>
                        <div className="p-4 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
                            <button onClick={() => setCloseTableModal({ open: false, tableNumber: null })} className="py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">Cancelar</button>
             ...
             {isWaiterModalOpen && (
                 <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full">
                        <div className="p-4 border-b"><h3 className="text-lg font-bold text-gray-800">Gestionar Mesoneros</h3></div>
                        <div className="p-6 max-h-80 overflow-y-auto">
                            <ul className="space-y-2 mb-4">
                                {waiters.map(waiter => (
                                    <li key={waiter} className="flex justify-between items-center p-2 bg-gray-100 rounded">
                                        <span className="text-gray-800 font-medium">{waiter}</span>
                            ...
                            <form onSubmit={handleAddWaiter} className="flex space-x-2">
                                <input type="text" value={newWaiterName} onChange={e => setNewWaiterName(e.target.value)} placeholder="Nuevo mesero" className="flex-grow px-3 py-2 border border-gray-300 rounded-lg" required />
                        ...
                        <div className="p-4 bg-gray-50 rounded-b-xl text-right">
                            <button type="button" onClick={() => setIsWaiterModalOpen(false)} className="py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">Cerrar</button>
                        </div>
                    </div>
                </div>
             )}
                ...
                <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full">
                        <form onSubmit={handleReassignSubmit}>
                             <div className="p-4 border-b">
                                <h3 className="text-lg font-bold text-gray-800">Reasignar Mesero (Mesa {reassignModal.tableNumber})</h3>
                                <p className="text-sm text-gray-500">Mesero actual: {reassignModal.currentWaiter}</p>
                            </div>
                            <div className="p-6">
                                <label htmlFor="new-waiter" className="block text-sm font-medium text-gray-700 mb-2">Nuevo Mesero:</label>
                                <select 
                                    id="new-waiter"
                                    value={newWaiterId}
                            ...
                            <div className="p-4 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
                                <button type="button" onClick={() => setReassignModal({ ...reassignModal, open: false })} className="py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">Cancelar</button>
